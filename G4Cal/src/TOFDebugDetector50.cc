...
{
	// deux blocs situés de part et d'autre du centre à 10cm symétriquement .
	// Ce sont des cubes qui comportent 50*50 cristaux de 25*0.5*0.5mm3
  ...
  this->SetScoringVolume(lCrystal);
	...
	G4double crystalRepeatY = 0.5 *mm; //aucun espace entre les cristaux
  ...
  pOptical = new G4PVPlacement(0,G4ThreeVector(-100. * mm, 0. * mm, 0. * mm), lOptical, "pOptical", logicWorld, false, 1, true);
	...
	for (G4int crystalZ = 0; crystalZ < crystalRepeatNumberZ; crystalZ++)
	{
		for (G4int crystalY = 0; crystalY < crystalRepeatNumberY; crystalY++) // crystal repeater -> optical unit
		{
			G4double crystal_OffsetY = crystalRepeatY * .5 * ( 1 - crystalRepeatNumberY ); //parce que le repeater commence à partir centre du lvolume parent
